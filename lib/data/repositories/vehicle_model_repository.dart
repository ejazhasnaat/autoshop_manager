...
class VehicleModelRepository {
  ...
  Future<List<VehicleModel>> getAllVehicleModels() async {
    return (_db.select(_db.vehicleModels)..orderBy([
  ...
  Future<VehicleModel?> getVehicleModelByMakeModel(
    String make,
    String model,
  ) async {
  ...
  Future<bool> updateVehicleModel(VehicleModel modelToUpdate) async {
    ...
    final updatedCount = await _db
        .update(_db.vehicleModels)
        .replace(modelToUpdate);
  ...
  Future<bool> deleteVehicleModel(String make, String model) async {
    final count = await (_db.delete(
      _db.vehicleModels,
    )..where((vm) => vm.make.equals(make) & vm.model.equals(model))).go();
    return count > 0;
  }
